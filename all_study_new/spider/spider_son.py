...
class FoodSon():
    ...
    def parser(self,content):
        ...
        print(item)
        return item
    async def mongo_store(self,item):
        pass
        res = await self.mongocli.insert(item)
        print(res)

    async def aio_run(self,url):
        print('当前的url是: {}'.format(url))
        html = await self.downlader(url)
        # print(html)
        item = self.parser(html)
        # time.sleep(random.random())
        await self.mongo_store(item)
    def run_task(self):
        aio_tasks = []
    ...
    def callback(self,ch, method, properties, body):
        ...
        # aio_tasks.append(self.aio_run(url))
        # print(aio_tasks)
        # loop.run_until_complete(asyncio.wait(aio_tasks))
        # aio_tasks.clear()
        # html = await self.downlader(url)
        # # print(html)
        # item = self.parser(html)
    ...
    # def main(self):
    #     try:
    #         _thread.start_new_thread(self.recive)
    #         _thread.start_new_thread(self.run_task)
    #     except:
    #         print("Error: 无法启动线程")
    #
    #     while 1:
